 ...
 * @param {float} montant : est le montant des frais.
 * @param {bool} desactiveBtn : permet de savoir si on doit afficher les boutons de modification ou pas.
 * @param {integer} id : est l'id des div.
 * @param {integer} idHF : est l'id des boutons hors forfait.
 */
function ajoutLigne(laDate, libelle, montant, desactiveBtn, id, idHF) {
    ...
    ligne.innerHTML += "<td><input id='idDate" + id +"' type='text' class='form-control' value='" + laDate + "'/></td>\n\
                        <td><input id='idLibelle" + id +"' type='text' class='form-control' value='" + libelle + "'/></td>\n\
                        <td><input id='idMontant" + id +"' type='text' class='form-control' value='" + montant + "'/></td>";
                        if(desactiveBtn){
                            ligne.innerHTML += "<td><button id='idBtnReport" + idHF +"' type='button' style='cursor:pointer; width: 100px; color:#000;' onclick='\n\
                            javascript:if (confirm(&quot;Voulez-vous vraiment reporter cette ligne?&quot;)){\n\
                            reportFraisHF(this.id, this.parentNode.parentNode.rowIndex);}'>Reporter</button><br><br>\n\
                            <button id='idBtnSupp" + idHF +"' type='button' style='cursor:pointer; width: 100px; color:#000;' onclick='javascript:\n\
                            if (confirm(&quot;Voulez-vous vraiment supprimer cette ligne?&quot;)){ suppFraisHF(this.id, this.parentNode.parentNode.rowIndex);}'>\n\
                            Supprimer</button></td>";
                        }
}
 ...
 * 
 * @param {integer} num : est le numéro des lignes dans Hors forfait.
 */
function effacer(num) {
    
    $('#idRepas').val("");//Efface les frais repas midi
 ...
 * 
 * @param  {string} idVisiteur : est l'id du visiteur passé en paramètre.
 */
...
function recupFraisVisit() {
    $('#lstMois').on('change', function (event) {
        ...
        //Récupération des informations "frais" du mois pour le commercial sélectionné
        $.post('./controleurs/c_importFraisAValid.php', {postId: idVisiteur, postMois: annMois}, function (donn) {
            ...
            //donn = donn.substring(1, donn.length);//Anlève l'espace de la case 0 du tableau
            ...
            if (donn !== "0") {
                ...
                repas.val(donnees[3].quantite);//Renvoi les frais repas midi
                ...
                if (donnees[9]) {
                    ...
                    while (donnees[k]) {
                        ...
                        ajoutLigne(donnees[k].date, donnees[k].libelle, donnees[k].montant, desactiveBtn, k, donnees[k].id);
 ...
 * française.
 * 
 * @param {string} uneDate : est la date en format anglais.
 * @returns {string} dateFr : est la date en format français.
 */
